...
class WebjutterUpdater(BasicWorker):
    ...
    def work(self):
        ...
        webjutter_datasources_file = (
                self.config.PATH_ROOT / "config/extensions/webjutter_datasources.json"
        )
        ...
        if (
                self.config.get("webjutter-search.url")
            ...
            try:
                response = requests.get(
                    webjutter_url + "api/overview", auth=(webjutter_user, webjutter_pw)
                )
            ...
            if response and response.status_code == 200:
                try:
                    ...
                    with webjutter_datasources_file.open("w") as f:
                        ...
                        self.log.info(
                            f"Updated Webjutter datasources json file at {webjutter_datasources_file}"
                        )
                        remove_old = False
                except JSONDecodeError:
                    self.log.error(
                        "Couldn't parse Webjutter datasource.json:", collections
                    )

        # Remove old file to indicate Webjutter is not available.
        if remove_old and os.path.isfile(webjutter_datasources_file):
            self.log.info(
                f"Couldn't reach the Webjutter server, removing old datasource file at {webjutter_datasources_file}"
            )
