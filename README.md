...
3. Click 'Install'. The extension should be installed and 4CAT will be restarted. See the log for restart status. You may have to restart manually.
      - Alternatively, `Control Panel -> Extensions` also lets you upload this repo as a `.zip`, or you can manually create a git repository for this project in 4cat's `config/extensions` folder (see `README.md` in 4cat's `config/extensions` directory).
4. Go to `Control Panel -> Settings -> Extensions`. Make sure 'Webjutter Extensions' is selected, and restart 4CAT (`Control Panel -> Settings -> Restart or Upgrade`).
